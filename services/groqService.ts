// services/groqService.ts - HYBRID APPROACH
...
// ============================================================================
// FOOTBALL DATA API (Primary Source)
// ============================================================================
...
async function searchFootballDataAPI(query: string): Promise<{ team: Team; players: Player[] } | null> {
  ...
  try {
    ...
    console.log(`✅ [Football Data API] Retrieved ${players.length} players for ${team.name}`);
...
async function fetchPlayerImageFromTheSportsDB(playerName: string): Promise<string | undefined> {
  try {
    ...
    // Fetch player details using ID (this endpoint still works reliably)
    ...
    if (data.players && data.players.length > 0) {
      ...
      // Prefer cutout image, then thumb, then default
...
async function searchTeam(query: string): Promise<GROQSearchResponse> {
  ...
  // STEP 2: Try Football Data API (Primary Source)
  verificationSteps.push('📡 Checking Football Data API...');
  let footballDataResult = await searchFootballDataAPI(searchQuery);
  
  if (footballDataResult) {
    console.log(`✅ [Football Data API] Found: ${footballDataResult.team.name}`);
    verificationSteps.push(`✅ Found team in Football Data API: ${footballDataResult.team.name}`);
    
    // STEP 3: Get achievements from knowledge base
    verificationSteps.push('📖 Fetching team achievements...');
    const achievements = await fetchTeamAchievements(footballDataResult.team.name);
    footballDataResult.team.majorAchievements = achievements;
    verificationSteps.push('✅ Achievements retrieved');
    
    // STEP 4: Enrich players with images from TheSportsDB
    verificationSteps.push('🖼️ Fetching player images from TheSportsDB...');
    const playersWithImages = await enrichPlayersWithImages(footballDataResult.players);
    ...
    return {
      ...
      teams: [footballDataResult.team],
      youtubeQuery: `${footballDataResult.team.name} highlights ${SEASON_YEAR}`,
      _metadata: {
        source: 'Football Data API + TheSportsDB Images',
        confidence: 95,
        season: CURRENT_SEASON,
        verified: true,
        hasSquad: true,
  ...
  verificationSteps.push('⚠️ Football Data API returned no results');
  
  // STEP 5: Fallback to knowledge base
  verificationSteps.push('📖 Falling back to Knowledge Base...');
  console.log(`⚠️ [Football Data API] Not found, checking knowledge base...`);
...
// ============================================================================
// HISTORICAL PLAYERS FUNCTION - FOR ENHANCED TEAM RESULTS
// ============================================================================
...
export const getHistoricalPlayers = async (
  ...
  // Knowledge base of legendary players
